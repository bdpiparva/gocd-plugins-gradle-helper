...
class ProjectBuilderSpec extends Specification {
    ...
    def "it creates the poms with licenses"() {
        ...
        ProjectData data = builder.project {
            configuration("runtime") {
                ...
                module("mod2") {
                    pom("pom3") {
                        license(ProjectDataFixture.MIT_LICENSE())
                        license(ProjectDataFixture.LGPL_LICENSE())
                    }
                }
            }
            configuration("test") { }
        ...
        data.configurations*.dependencies.flatten().poms.flatten().find { it.name == "pom1" }.licenses as List == [ProjectDataFixture.APACHE2_LICENSE()]
        data.configurations*.dependencies.flatten().poms.flatten().find { it.name == "pom2" }.licenses as List == [ProjectDataFixture.MIT_LICENSE()]
        data.configurations*.dependencies.flatten().poms.flatten().find { it.name == "pom3" }.licenses as List == [ProjectDataFixture.MIT_LICENSE(), ProjectDataFixture.LGPL_LICENSE()]
    ...
    def "it creates the poms license with custom name and url"() {
        ...
        ProjectData data = builder.project {
            configuration("runtime") {
                module("mod1") {
                    ...
                    pom("pom2") {
                        license(url: "123")
                        license(ProjectDataFixture.LGPL_LICENSE(), name: "lgpl123")
                    }
                }
                module("mod2") {
                    pom("pom3") {
        ...
        data.configurations*.dependencies.flatten().poms.flatten().find { it.name == "pom2" }.licenses*.url == ["123", ProjectDataFixture.LGPL_LICENSE().url]
        data.configurations*.dependencies.flatten().poms.flatten().find { it.name == "pom3" }.licenses*.name == [ProjectDataFixture.LGPL_LICENSE().name]
    ...
    def "it creates manifests with licenses"() {
        ...
        ProjectData data = builder.project {
            configuration("runtime") {
                module("mod1") {
                    manifest("mani1") {
                        license(ProjectDataFixture.APACHE2_LICENSE().name)
                    }
                    manifest("mani2") {
                        license(ProjectDataFixture.MIT_LICENSE())
                    }
                ...
                module("mod2") {
                    manifest("mani3") {
                        license(ProjectDataFixture.LGPL_LICENSE().url)
                    }
                }
            }
            configuration("test") { }
        ...
        data.configurations*.dependencies.flatten().manifests.flatten().find { it.name == "mani1" }*.license == [ProjectDataFixture.APACHE2_LICENSE().name]
        data.configurations*.dependencies.flatten().manifests.flatten().find { it.name == "mani2" }*.license == [ProjectDataFixture.MIT_LICENSE().name]
        data.configurations*.dependencies.flatten().manifests.flatten().find { it.name == "mani3" }*.license == [ProjectDataFixture.LGPL_LICENSE().url]
