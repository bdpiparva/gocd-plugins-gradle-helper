...
class LicenseDataCollectorSpec extends Specification {
    ...
    def "keep manifest-license when name/url not matches a existing licenses name or url"() {
        ...
        then:
        com.github.jk1.license.ProjectBuilder.json(result) == """{
    "moduleUrls": [
        "http://dummy-mani-url",
        ...
        ProjectData projectData = builder.project {
            configuration("runtime") {
                module("mod1") {
                    ...
                    manifest("mani2") {
                        license(ProjectDataFixture.APACHE2_LICENSE().url)
                    }
                }
            }
        }
        ...
        ProjectData projectData = builder.project {
            configuration("runtime") {
                module("mod1") {
                    ...
                    licenseFiles {
                        licenseFileDetails(file: "apache2-license.txt", license: "Apache 2.0", licenseUrl: ProjectDataFixture.APACHE2_LICENSE().url)
                    }
                }
            }
        }
        ...
        ProjectData projectData = builder.project {
            configuration("runtime") {
                module("mod1") {
                    ...
                    licenseFiles {
                        licenseFileDetails(file: "apache2-license.txt", license: ProjectDataFixture.APACHE2_LICENSE().name, licenseUrl: ProjectDataFixture.APACHE2_LICENSE().url)
                        licenseFileDetails(file: "apache2-license.txt", license: ProjectDataFixture.APACHE2_LICENSE().name, licenseUrl: "http://some-url")
                    }
                }
            }
        }
