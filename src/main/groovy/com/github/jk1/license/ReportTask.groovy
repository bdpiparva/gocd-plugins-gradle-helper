...
class ReportTask extends DefaultTask {
    ...
    String getConfigurationSnapshot(){
        return getProject().gocdPlugin.licenseReport.snapshot
    }

    @OutputDirectory
    File getOutputFolder(){
        return new File(getProject().gocdPlugin.licenseReport.outputDir)
    }

    @TaskAction
    void generateReport() {
        LOGGER.info("Processing dependencies for project ${getProject().name}")
        LicenseReport config = getProject().gocdPlugin.licenseReport
